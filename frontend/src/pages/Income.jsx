...
const TAX_GROUP_LABELS = {
  ordinary:     "Ordinary Income (W2 + 1099)",
  pass_through: "Pass-Through (K-1)",
  rental:       "Rental Income",
  investment:   "Investment",
  other:        "Other (refunds, reimbursements, misc)",
};
...
function IncomeCategoryCell({ txn, categories, onSave }) {
  const [editing,  setEditing]  = useState(false);
...
function IncomeLabelCell({ txn, labels, onSave }) {
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
          ...
          <div>
            ...
            <input
              list="qr-income-cat-list"
              value={cat}
              onChange={e => setCat(e.target.value)}
              style={qrInputStyle}
            />
            <datalist id="qr-income-cat-list">
          ...
          <div>
            ...
            <input
              list="qr-income-label-list"
              value={lbl}
              onChange={e => setLbl(e.target.value)}
              style={qrInputStyle}
              placeholder="e.g. Gym"
            />
            <datalist id="qr-income-label-list">
...
const PIE_COLORS = ["#2C4A3B","#4A7C59","#7BAE8C","#A8D5B5","#3D6350","#6FAF85","#1F3A2E"];
...
function MonthlyChart({ data, selectedMonth }) {
  const byMonth = {};
        ...
        return (
          <g key={m}>
            <rect x={x} y={padY + H - barH} width={barW} height={barH} rx="3"
              fill={isSelected ? "#1F3A2E" : "#2C4A3B"}
              opacity={dimmed ? 0.25 : isSelected ? 1 : 0.8} />
            <text x={x + barW / 2} y={H + padY + 18} fontFamily="Geist Mono" fontSize="10"
              fill={isSelected ? "#2C4A3B" : dimmed ? "#C5C9C5" : "#8A908A"} textAnchor="middle">{MONTH_LABELS[i]}</text>
            {values[i] > 0 && !dimmed && (
...
export default function Income() {
  ...
  useEffect(() => {
    fetch(`/api/income/summary?period=${period}`).then(r => r.json()).then(setSummary);
    fetch(`/api/income/tax-exposure?year=${year}`).then(r => r.json()).then(setTaxExp);
    fetch("/api/income/yoy").then(r => r.json()).then(setYoy);
  }, [period]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/income/monthly?${params}`).then(r => r.json()).then(setMonthly);
  ...
  useEffect(() => {
    ...
    fetch(`/api/income/transactions?${params}`)
      .then(r => r.json())
    ...
    <main className="main">
      ...
      <div className="topbar">
        <div>
          <div className="breadcrumb">
            <span>Household</span><span className="sep">/</span><span>Income</span>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          {(() => {
            ...
            const filteredTotal = hasFilter
              ...
              : summary.total;
            return (
              <div className="card" style={{ background: "var(--brand-700)", color: "var(--paper-0)", border: 0 }}>
                <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", opacity: 0.65, fontWeight: 500 }}>
                  {hasFilter ? "Filtered income" : "Total income"} · {period}
                </div>
                ...
                return (
                  <button key={t.category} onClick={() => toggleCategory(t.category)}
                    style={{
                      ...
                      border: "1px solid",
                      borderColor: active ? "var(--brand-400)" : "var(--paper-3)",
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div>
            <div className="stat-label">Monthly income · {period}</div>
          </div>
        </div>
        <MonthlyChart data={monthly} selectedMonth={month} />
      </div>
      ...
      <div className="card flush" style={{ marginBottom: 16 }}>
        <div className="card-head">
          <div>
            <div className="card-title">Income transactions</div>
          ...
          <label className="input" style={{ minWidth: 220 }}>
            <input
              ...
              placeholder="Search… label:Bolinas category:Income amount:7600 merchant:check"
          ...
          <tbody>
            {txns.map(txn => (
              <tr key={txn.id}>
                ...
                <td>
                  <IncomeCategoryCell txn={txn} categories={categories} onSave={handleCategorySave} />
                ...
                <td>
                  <IncomeLabelCell txn={txn} labels={labels} onSave={handleLabelSave} />
                </td>
                <td className="num money mono" style={{ fontWeight: 500, color: "var(--income)" }}>
                  {fmt(txn.amount)}
                </td>
                <td>
                  <button
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                return (
                  <tr key={y} style={{ borderTop: "1px solid var(--paper-2)" }}>
                    ...
                    <td style={{ textAlign: "right", padding: "8px 0" }}>
                      {delta !== null && (
                        <span style={{ fontSize: 12, color: delta >= 0 ? "var(--income)" : "var(--expense)" }}>
        ...
        <div className="card">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            <div className="stat-label">Tax exposure · {year}</div>
            <span style={{ fontSize: 11, color: "var(--ink-3)" }}>categorization aid, not tax advice</span>
          </div>
          {taxExp ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {Object.entries(taxExp).map(([group, data]) => (
                data.total > 0 && (
                  <div key={group} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
                    <div>
                      <div style={{ fontWeight: 500 }}>{TAX_GROUP_LABELS[group]}</div>
                      <div style={{ fontSize: 11, color: "var(--ink-3)" }}>{data.count} transactions</div>
                    </div>
                    <div className="mono" style={{ fontWeight: 500 }}>{fmt(data.total)}</div>
                  </div>
                )
              ))}
              {Object.values(taxExp).every(d => d.total === 0) && (
                <p style={{ fontSize: 13, color: "var(--ink-3)", margin: 0 }}>
                  Assign source types to transactions to see tax groupings.
                </p>
              )}
            </div>
          ) : (
